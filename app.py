...
if estudiante:
    with st.expander("✏️ Editar datos del estudiante", expanded=False):
        ...
        # Keys dinámicos según estudiante
        with ec1:
            nuevo_codigo = st.text_input("Código", str(fila["Código"]), key=f"edit_codigo_{estudiante}")
        with ec2:
            nuevo_nombre = st.text_input("Nombre", str(fila["Nombre"]), key=f"edit_nombre_{estudiante}")
        with ec3:
            nuevo_apellido = st.text_input("Apellidos", str(fila["Apellidos"]), key=f"edit_apellido_{estudiante}")
        with ec4:
            try:
                idx_frat = FRATERNIDADES.index(str(fila["Fraternidad"]))
            except ValueError:
                idx_frat = 0
            nueva_fraternidad = st.selectbox(
                "Fraternidad", FRATERNIDADES, index=idx_frat, key=f"edit_frat_{estudiante}"
            )

        if st.button("💾 Guardar cambios", key=f"btn_save_edit_{estudiante}"):
...
if estudiante:
    with st.expander("➕➖ Asignar puntos", expanded=st.session_state.get("abrir_puntos", False)):
        ...
        with pc1:
            columna = st.selectbox("Categoría", CATEGORIAS, key=f"puntos_categoria_{estudiante}")
        with pc2:
            puntos = st.number_input("Puntos (+/-)", step=1, value=1, min_value=-10, max_value=10,
                                     key=f"puntos_valor_{estudiante}")
        with pc3:
            accion_rapida = st.radio(
                "Acción rápida", ["Ninguna", "+1", "+5", "-1", "-5"],
                index=0, horizontal=True, key=f"puntos_rapidos_{estudiante}"
            )
        ...
        if st.button("Actualizar puntos", key=f"btn_actualizar_puntos_{estudiante}"):
            if -10 <= puntos <= 10:
